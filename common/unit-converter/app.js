const domains = {
    ...
    physics: {
        categories: {
            ...
            wavelength: { name: '파장' },
            density: { name: '밀도' },
            radioactivity: { name: '방사능' },
            radiation_dose: { name: '방사선량' }
        }
    },
    chemistry: {
        categories: {
...
const unitsData = {
    ...
    area: {
        ...
        'ac': { name: '에이커 (ac)', factor: 4046.856 },
    ...
    volume: {
        ...
        'gal': { name: '미국 갤런 (US gal)', factor: 3.78541 },
        'fl_oz': { name: '미국 액량 온스 (US fl oz)', factor: 0.0295735 }
    },
    mass: {
        ...
        'oz': { name: '온스 (oz)', factor: 0.02834952 }
    },
    time: {
        's': { name: '초 (s)', factor: 1 },
    ...
    force: {
        ...
        'lbf': { name: '파운드힘 (lbf)', factor: 4.44822 }
    },
    pressure: {
    ...
    energy: {
        ...
        'cal': { name: 'cal_th 또는 열화학 cal', factor: 4.184 },
    ...
    power: {
        ...
        'hp': { name: '마력, mechanical hp', factor: 745.7 }
    },
    electricity: {
    ...
    radioactivity: {
        'Bq': { name: '베크렐 (Bq)', factor: 1 },
        'kBq': { name: '킬로베크렐 (kBq)', factor: 1000 },
        'MBq': { name: '메가베크렐 (MBq)', factor: 1000000 },
        'Ci': { name: '퀴리 (Ci)', factor: 3.7e10 }
    },
    radiation_dose: {
        'Gy': { name: '그레이 (Gy)', factor: 1 },
        'mGy': { name: '밀리그레이 (mGy)', factor: 0.001 },
        'Sv': { name: '시버트 (Sv)', factor: 1 },
        'mSv': { name: '밀리시버트 (mSv)', factor: 0.001 }
    },
    ...
    chem_volume: {
        'L': { name: '리터 (L)', factor: 1 },
        'mL': { name: '밀리리터 (mL)', factor: 0.001 },
        'cm3': { name: '세제곱센티미터 (cm³)', factor: 0.001 },
    ...
    astronomy_dist: {
        ...
        'pc': { name: '파섹 (pc)', factor: 3.26156 },
        'AU': { name: '천문단위 (AU)', factor: 0.0000158125 },
        'km': { name: '킬로미터 (km)', factor: 1.057e-13 }
    },
    cosmic_speed: {
    ...
    angle: {
        ...
        'radian': { name: '라디안 (rad)', factor: 57.2957795 },
...
function populateSelects() {
    ...
    if (secondUnit) {
        outputUnitSelect.value = secondUnit;
    }
}
...
function formatExponent(str) {
    ...
    const superscriptExponent = exponent.split('').map(char => superscriptMap[char] || char).join('');
...
function formatFloat(num) {
    ...
    // 너무 작거나 너무 큰 값은 과학적 표기법 사용
    if (abs < 0.000001 || abs >= 1000000000) {
        let eStr = num.toExponential(6).replace(/\.?0+e/, 'e');
        return formatExponent(eStr);
    }
...
function convert() {
    ...
    if (currentCategory === 'temperature') {
        ...
        if (fromKey === 'celsius') {
            celsius = val;
            if (toKey === 'celsius') {
                result = val; fText = `동일 단위 변환입니다.`; cProc = `${val} = ${val}`;
        ...
        let ratioStr = ratio.toPrecision(6).replace(/\.?0+e/, 'e').replace(/\.?0+$/, '');
        if (ratioStr.endsWith('.')) ratioStr = ratioStr.slice(0, -1);
        
        ratioStr = formatExponent(ratioStr);
